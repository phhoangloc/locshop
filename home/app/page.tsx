...
export default function Home() {
  ...
  const [_blogs, set_blogs] = useState<{ name: string, cover: string, category: { name: string }, content: string }[]>([])
  ...
  useEffect(() => {
    ...
    (async () => {
      const result = await axios.get("http://localhost:4000/api/blog")
      set_blogs(result.data.data)

    })();
    (async () => {
      const result = await axios.get("http://localhost:4000/api/product?limit=3")
  ...
  return (
    <div>
      <Cover name={_name} image={_image} description={_description} />
